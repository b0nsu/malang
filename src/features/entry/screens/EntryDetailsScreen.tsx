...
export default function EntryDetailsScreen() {
  ...
  const save = async () => {
    ...
    try {
      await saveEntry({ date: draft.date, emotionId: draft.emotionId, value: draft.value, note: draft.note || null }, { version: 1, parameters: draft.face, appearance }, { createdAt: new Date().toISOString(), timezoneOffsetMinutes: new Date().getTimezoneOffset(), retrospectiveFlag: isRetrospectiveDate(draft.date) });

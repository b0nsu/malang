...
export function clampFace(face:FaceParametersV1):FaceParametersV1 { return {...face,brows:{left:{centerY:clamp(face.brows.left.centerY),outerY:clamp(face.brows.left.outerY)},right:{centerY:clamp(face.brows.right.centerY),outerY:clamp(face.brows.right.outerY)}},eyes:{left:{...face.eyes.left,openness:clamp(face.eyes.left.openness,0,1),tilt:clamp(face.eyes.left.tilt),scaleX:clamp(face.eyes.left.scaleX,0,2),scaleY:clamp(face.eyes.left.scaleY,0,2)},right:{...face.eyes.right,openness:clamp(face.eyes.right.openness,0,1),tilt:clamp(face.eyes.right.tilt),scaleX:clamp(face.eyes.right.scaleX,0,2),scaleY:clamp(face.eyes.right.scaleY,0,2)}},mouth:{leftCornerY:clamp(face.mouth.leftCornerY),rightCornerY:clamp(face.mouth.rightCornerY),openness:clamp(face.mouth.openness,0,1)},face:{width:clamp(face.face.width),length:clamp(face.face.length),skewX:clamp(face.face.skewX),tilt:clamp(face.face.tilt),volume:clamp(face.face.volume)}}; }
export type FaceAxis = keyof FaceParametersV1['face'];
export function adjustFaceAxis(face: FaceParametersV1, axis: FaceAxis, delta: number) { return clampFace({ ...face, face: { ...face.face, [axis]: face.face[axis] + delta } }); }
